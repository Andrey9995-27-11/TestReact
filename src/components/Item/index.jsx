...
const Item = (props) => {
  const removeBtnVisible =
    props.basketEvent === BASKET_REMOVE ? props.basketEvent : false

  return (
    <li
      className={'list-item  list-item--' + props.result.wrapperType}
      item-id={props.result.ID}
    >
      <Link to={'/detail/' + props.result.ID} className="list-item__inner">
        <RemoveButton
          toRemoveHandler={props.toRemoveHandler}
          visible={removeBtnVisible}
        ...
        <div className="list-item__content">
          <div className="list-item__title">{props.result.NAME}</div>
          <div className="list-item__desc">{props.result.PRICE + '$'}</div>
        </div>
        <Button
          basketChange={props.basketChange}
          basketEvent={props.basketEvent}
          result={props.result}
        />
      </Link>
    </li>
